...
const checkDogs = function (dogsJulia, dogsKate) {
  const tempArr = dogsKate.slice();
  tempArr.splice(0, 1);
  tempArr.splice(-2, 2);

  const combined = [...tempArr, ...dogsJulia];
...
const calcAverageHumanAge = function (ages) {
  const ageInHumanYears = ages.map(age => (age <= 2 ? 2 * age : 16 + age * 4));
  const adultDogs = ageInHumanYears.filter(curr => curr >= 18);
...
too little!"
 */
// const answer = ownersEatTooLittle.reduce((acc, curr, i, arr) => {
//   if (i !== arr.length - 1) {
//     acc = `${acc}${curr} and `;
//   } else {
//     acc = `${acc}${curr}'s dogs eat too little.`;
//   }

//   return acc;
// }, '');
// const answer2 = ownersEatTooMuch.reduce((acc, curr, i, arr) => {
//   if (i !== arr.length - 1) {
//     acc = `${acc}${curr} and `;
//   } else {
//     acc = `${acc}${curr}'s dogs eat too much.`;
//   }
...
that is recommended (just true or false)
 */
const answer3 = dogs.some(dog => dog.curFood === dog.recommendedFood);
console.log(answer3);
